...
def open_session(session_id: str) -> tuple[Any, str]:
    """Return (session_db, resolved_session_id) for the given session.

    Resolves session-chain aliases via Hermes's `resolve_resume_session_id`
    so callers always operate on the live session ID.
    """
